...
def main():
    ...
    sessions = asyncio.run(
        runner.session_service.list_sessions(app_name=runner.app_name)
    )

    # Example Usage
    result_b = run_coordinator(
        runner, "What is the highest mountain in the world?", user_id, session_id
    )
    result_a = run_coordinator(runner, "Book me a hotel in Paris.", user_id, session_id)
    print(f"Final Output A: {result_a}")
    print(f"Final Output B: {result_b}")
    result_c = run_coordinator(
        runner, "Tell me a random fact.", user_id, session_id
    )  # Should go to Info
    print(f"Final Output C: {result_c}")
    result_d = run_coordinator(
